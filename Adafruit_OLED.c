...
*/
    ...
    GPIOPinWrite(DC_BASE, DC_OFFSET, 0); //DC
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_OFFSET, 0); //DISABLE OLEDCS
    //SPITransfer(GSPI_BASE,&c,0,50,
                    //SPI_CS_ENABLE|SPI_CS_DISABLE);
    MAP_SPIDataPut(GSPI_BASE, c);
    MAP_SPIDataGet(GSPI_BASE, &ulDummy);
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_OFFSET, OLEDCS_OFFSET); //ENABLE OLEDCS
...
*/
    unsigned long ulDummy;
    MAP_SPICSEnable(GSPI_BASE);
    GPIOPinWrite(DC_BASE, DC_OFFSET, DC_OFFSET); //DC
    GPIOPinWrite(OLEDCS_BASE, OLEDCS_OFFSET, 0); //DISABLE OLEDCS
    //SPITransfer(GSPI_BASE,&c,0,50,
                    //SPI_CS_ENABLE|SPI_CS_DISABLE);
    MAP_SPIDataPut(GSPI_BASE, c);
...
*/
  ...
  GPIOPinWrite(OLEDCS_BASE, OLEDCS_OFFSET, OLEDCS_OFFSET); /// PIN 18 OLEDCS
  // OUR RESET PIN IS PIN_03

  volatile unsigned long delay;

  GPIOPinWrite(R_BASE, R_OFFSET, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(R_BASE, R_OFFSET, R_OFFSET);	// RESET = RESET_HIGH
